...
class FileText: public WidgetDecorator{
  ...
  size_t drawLineWithWrap(const std::string &s, size_t y, const State &state);
...
public:
  ...
  FileText(const std::unordered_map<int, int> &syntaxColorMap, std::unique_ptr<Widget> widget):
    WidgetDecorator(std::move(widget)), currentLine{0}, screenCursorPos{0,0}, syntaxColorMap(syntaxColorMap){}
